...
METACHUNK_COUNT = 2  # Number of chunks to combine into a metachunk
FILE_COUNT = 3  # Number of files to split metachunks into
        ...
        try:
            with open(file_path, 'r', encoding=encoding) as file:
                ...
                if detect_language(text[:1000]) == "Ancient Greek":
                    ...
                    print(f"Estimated blank lines: {blank_lines}")
...
def split_into_paragraphs(text):
    ...
    This is the core function that determines how many segments we'll have.
    """
    ...
    print(f"Total paragraphs after splitting: {len(paragraphs)}")
...
def write_segments(segments, output_dir, corpus_name, prefix="segment"):
    ...
    # Prepare data for CSV with normalized formatting
    segments_data = []
    
    # Write each segment to a file with UTF-8 encoding
    for i, segment in enumerate(segments, 1):
        # Normalize whitespace - replace any contiguous whitespace with a single space
        ...
        with open(output_file, 'w', encoding='utf-8') as file:
            file.write(normalized_segment)
        print(f"Wrote {prefix} {i} to {output_file} ({len(normalized_segment)} characters)")
        
        # Add to segments data for CSV
        segments_data.append({
            'sequence_no': i,
            'sequence_text': segment
        })
    
    # Create combined CSV file with all segments
    csv_file = output_dir / f"all_{prefix}s_{corpus_name}.csv"
    df_segments = pd.DataFrame(segments_data)
    df_segments.to_csv(csv_file, index=False, quoting=csv.QUOTE_NONNUMERIC)
    print(f"Wrote all segments to CSV file: {csv_file}")

def create_metachunks(segments, metachunk_count):
    """
    Create metachunks by combining adjacent segments.
    
    Args:
        segments: List of segment texts
        metachunk_count: Number of segments to combine into one metachunk
        
    Returns:
        DataFrame with metachunk ID and text
    """
    print(f"\nCreating metachunks from {len(segments)} segments with {metachunk_count} segments per metachunk")
    
    if metachunk_count <= 1:
        # Just convert segments to DataFrame if no combining needed
        print(f"Metachunk count is {metachunk_count}, so each segment becomes its own metachunk")
        df_meta = pd.DataFrame({
            'metachunk_id': range(1, len(segments) + 1),
            'metachunk_text': segments
        })
        print(f"Created {len(df_meta)} metachunks")
        return df_meta
    
    metachunks = []
    metachunk_ids = []
    
    # Group segments into metachunks
    for i in range(0, len(segments), metachunk_count):
        # Get segments for this metachunk
        chunk_group = segments[i:i+metachunk_count]
        
        # Combine with triple newlines between segments
        metachunk_text = "\n\n\n".join(chunk_group)
        
        # Add to lists
        metachunks.append(metachunk_text)
        metachunk_ids.append(i // metachunk_count + 1)  # 1-based index
    
    # Create DataFrame
    df_meta = pd.DataFrame({
        'metachunk_id': metachunk_ids,
        'metachunk_text': metachunks
    })
    
    print(f"Created {len(metachunks)} metachunks by combining {metachunk_count} segments each")
    
    return df_meta

def split_and_write_metachunks(df_meta, corpus_name, file_count, output_base_dir):
    """
    Split metachunks into multiple files and write them.
    
    Args:
        df_meta: DataFrame with metachunks
        corpus_name: Name of the corpus
        file_count: Number of files to split into
        output_base_dir: Base directory for output
    """
    # Format the text content - one segment per line with normalized whitespace
    df_meta['formatted_text'] = df_meta['metachunk_text'].apply(
        lambda text: ' '.join(re.sub(r'\s+', ' ', segment).strip() for segment in text.split('\n\n\n'))
    )
    
    # Rename columns for CSV output
    df_meta_csv = df_meta.rename(columns={
        'metachunk_id': 'sequence_no',
        'metachunk_text': 'sequence_text'
    }).drop(columns=['formatted_text'])
    
    # Create output directory - fix the path structure
    # Use Path('../data') as the base, then add corpus_name and documents_meta
    output_dir = Path('../data') / corpus_name / "documents_meta"
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"Writing metachunks to directory: {output_dir}")
    print(f"Total number of metachunks: {len(df_meta)}")
    
    if file_count <= 1:
        # If only one file requested, write all metachunks to it
        # Write text file with one segment per line
        output_file = output_dir / f"meta1_{corpus_name}.txt"
        with open(output_file, 'w', encoding='utf-8') as file:
            file.write('\n'.join(df_meta['formatted_text']))
        
        # Write CSV file with specified format
        csv_file = output_dir / f"meta1_{corpus_name}.csv"
        df_meta_csv.to_csv(csv_file, index=False, quoting=csv.QUOTE_NONNUMERIC)
        
        print(f"Wrote all {len(df_meta)} metachunks to single file: {output_file}")
        print(f"Wrote all {len(df_meta)} metachunks to CSV file: {csv_file}")
        return
    
    # Calculate chunks per file (distribute evenly)
    total_chunks = len(df_meta)
    base_chunks_per_file = total_chunks // file_count
    remainder = total_chunks % file_count
    
    print(f"Distributing {total_chunks} metachunks across {file_count} files")
    print(f"Base chunks per file: {base_chunks_per_file}, remainder: {remainder}")
    
    start_idx = 0
    
    # For each file
    for file_idx in range(1, file_count + 1):
        # Calculate how many chunks go in this file
        # Add an extra chunk if we have remainder chunks to distribute
        chunks_this_file = base_chunks_per_file + (1 if file_idx <= remainder else 0)
        
        if chunks_this_file == 0:
            print(f"Warning: File {file_idx} would have no chunks, skipping")
            continue
            
        end_idx = start_idx + chunks_this_file
        
        # Get chunks for this file
        file_chunks = df_meta.iloc[start_idx:end_idx]
        file_chunks_csv = df_meta_csv.iloc[start_idx:end_idx]
        
        # Write to text file - one segment per line
        output_file = output_dir / f"meta{file_idx}_{corpus_name}.txt"
        with open(output_file, 'w', encoding='utf-8') as file:
            file.write('\n'.join(file_chunks['formatted_text']))
        
        # Write to CSV file with specified format
        csv_file = output_dir / f"meta{file_idx}_{corpus_name}.csv"
        file_chunks_csv.to_csv(csv_file, index=False, quoting=csv.QUOTE_NONNUMERIC)
        
        print(f"Wrote file {file_idx} with {chunks_this_file} metachunks to {output_file}")
        print(f"Wrote file {file_idx} with {chunks_this_file} metachunks to CSV file {csv_file}")
        start_idx = end_idx
...
def parse_arguments():
    ...
    parser.add_argument('--metachunk-count', '-m', type=int, default=METACHUNK_COUNT,
                        help=f'Number of chunks to combine into a metachunk (default: {METACHUNK_COUNT})')
    
    parser.add_argument('--file-count', '-f', type=int, default=FILE_COUNT,
                        help=f'Number of files to split metachunks into (default: {FILE_COUNT})')
    
    parser.add_argument('--skip-metachunks', action='store_true',
                        help='Skip creation of metachunks')
...
def main():
    ...
    if not args.analyze_only:
        ...
        # Create metachunks if requested
        if not args.skip_metachunks:
            # Create metachunks DataFrame (will just contain the original segments if metachunk_count=1)
            df_meta = create_metachunks(segments, args.metachunk_count)
            
            # Process metachunks
            split_and_write_metachunks(df_meta, args.corpus, args.file_count, args.output)
            print(f"Metachunk processing complete")
    else:
