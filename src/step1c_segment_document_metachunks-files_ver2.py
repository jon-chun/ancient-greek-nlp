...
def write_segments(segments, output_dir, corpus_name, prefix="segment"):
    ...
    pd.DataFrame(segments_data).to_csv(csv_file, index=False)
...
def create_metachunks(segments, metachunk_count):
    ...
    print(f"\nCreated {len(metachunks)} metachunks by combining {metachunk_count} segments each")
...
def split_and_write_metachunks(df_meta, corpus_name, file_count, output_base_dir):
    ...
    if file_count <= 1:
        ...
        output_dir = output_base_dir.parent / corpus_name / "documents"
        os.makedirs(output_dir, exist_ok=True)
        
        output_file = output_dir / f"meta1_{corpus_name}.txt"
        with open(output_file, 'w', encoding='utf-8') as file:
            file.write("\n\n\n\n".join(df_meta['metachunk_text']))
        
        # Also write a CSV version with all metachunks
        csv_file = output_dir / f"meta1_{corpus_name}.csv"
        df_meta.to_csv(csv_file, index=False)
        
        print(f"Wrote all metachunks to single file: {output_file}")
        print(f"Wrote all metachunks to CSV file: {csv_file}")
        return
    ...
    for file_idx in range(1, file_count + 1):
        ...
        with open(output_file, 'w', encoding='utf-8') as file:
            file.write("\n\n\n\n".join(file_chunks['metachunk_text']))
        
        # Write to CSV file
        csv_file = output_dir / f"meta{file_idx}_{corpus_name}.csv"
        file_chunks.to_csv(csv_file, index=False)
...
def main():
    ...
    if not args.analyze_only:
        ...
        if not args.skip_metachunks:
            ...
            # Only process further if metachunk count > 1
            if args.metachunk_count > 1:
                # Split and write metachunks to files
